        ...
        for (ifaptr = ifap; ifaptr != nullptr; ifaptr = (ifaptr)->ifa_next) {
            if(ifaptr->ifa_addr!= nullptr) {
                ...
                if (family == AF_PACKET) {
                    ...
                    mac = mac+"AF_PACKET"+(ifaptr)->ifa_name+macp;
...
//                    }
                    ...
                    if (ret != 0) {
                        mac = mac+"AF_INET6  getnameinfo() failed   %s  "+gai_strerror(ret);
                        LOGE("AF_INET6  getnameinfo() failed   %s  ",gai_strerror(ret))
                    }
                    mac =  mac+"AF_INET6 %s %s  "+(ifaptr)->ifa_name+host;
...
Java_com_doctor3_propertytest_MainActivity_getmac(JNIEnv *env, jclass clazz) {
    listmacaddrs();
